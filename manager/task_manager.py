...
class TaskManager:    
    ...
    def calculate_meta_feature(self, eval_func: Callable, task_id: str = "default", **kwargs):
        ...
        # import json
        # import pickle
        # # 从 /root/codes/exp_results/tpch_600g_nohis/locat/extract_configs.json 中读取extract_configs
        # with open("/root/codes/exp_results/tpch_600g_nohis/locat/extract_configs.json", "r", encoding="utf-8") as f:
        #     extract_configs = json.load(f)
        
        # extract_configs = [config for config, _ in extract_configs]

        # results = []
        # for config in extract_configs[::-1]:
        #     # 修改 "spark.driver.maxResultSize": 5104
        #     config["spark.driver.maxResultSize"] = 2048
        #     results.append(eval_func(config=config, resource_ratio=1.0))
            
        #     # 保存到pkl
        #     with open("/root/codes/exp_results/tpch_600g_nohis/locat/extract_configs_results.pkl", "wb") as f:
