...
from .calculate import multi_fidelity_sql_selection
...
class PartitionPlan:
    ...
    sql_estimated_times: Dict[str, float] = field(default_factory=dict)
...
class SQLPartitioner(Partitioner[PartitionPlan]):
    ...
    def build_plan(
        ...
        *,
        ...
        fidelity_subsets, sql_estimated_times, fidelity_correlations = multi_fidelity_sql_selection(
        ...
        if not fidelity_subsets:
            logger.warning("SQLPartitioner: no SQL subsets generated.")
            return self._create_empty_plan("no_subsets")

        subset_correlations: Dict[float, float] = fidelity_correlations

        from .utils import custom_sort
        
        plan = PartitionPlan(
            fidelity_subsets={k: sorted(values, key=custom_sort) for k, values in fidelity_subsets.items()},
            sql_estimated_times=sql_estimated_times,
            metadata={
        ...
        *,
        ...
        if include_current_task:
            ...
            if current_history is not None and len(current_history) > 0:
                logger.info(f"SQLPartitioner: current history: {current_history.task_id}")
        ...
        if similar_histories:
            for history, (_, score) in zip(similar_histories, similar_scores):
                weight = float(score)
                if len(history) > 0:
                    logger.info(f"SQLPartitioner: similar history: {history.task_id}, score: {score}")
                    histories.append((history, weight))
        logger.info(f"SQLPartitioner: collected {len(histories)} histories")
        for history, weight in histories:
            logger.info(f"SQLPartitioner: history: {history.task_id}, weight: {weight}")
        return histories
