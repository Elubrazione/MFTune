...
from .acq_function.optimizer import create_local_random_optimizer, CompositeOptimizer
from .acq_function.optimizer.generator import LocalSearchGenerator, RandomSearchGenerator
from .acq_function.optimizer.selector import InterleavedSelector, ProbabilisticSelector
...
class BO(BaseAdvisor):
    def __init__(self, config_space: ConfigurationSpace, method_id='unknown',
        ...
        logger.info("BO: rand_prob: %f" % self.rand_prob)
        self.acq_optimizer = CompositeOptimizer(
            acquisition_function=self.acq_func,
            config_space=self.sample_space,
            strategies=[local_strategy, random_strategy],
            selector=ProbabilisticSelector([1 - self.rand_prob, self.rand_prob], self.rng),
            rng=self.rng,
            candidate_multiplier=3.0
        )
    ...
    def warm_start(self):
        if self.already_warm_start:
            return
    
        self.already_warm_start = True
    ...
    def sample(
        ...
        if not self.already_warm_start:
            self.warm_start()
            logger.info("BO: Begin to warm start!")
        ...
        challengers = self.acq_optimizer.maximize(
            ...
            num_points=batch_size - len(batch),
        ...
        for config in challengers:
            ...
            if self.validation_strategy.is_valid(config):
                ...
                batch.append(config)
                logger.info("BOHB/MFES: take config from acquisition function: %s" % config.origin)
