...
class BestConfigsWarmStart(WarmStarter):
    ...
    def get_initial_configs(
        ...
        ini_list = []
        logger.info(f"Warm_start_nums: init_num({init_num})")
        init_num = init_num * self.ws_topk
        
        already_nums = num_evaluated
        for rank in range(self.ws_topk):
            if len(ini_list) + num_evaluated >= init_num:
                break
        ...
        ini_list = ini_list[::-1]        
        while len(ini_list) + num_evaluated < init_num:
...
class RandomConfigsWarmStart(WarmStarter):
    def __init__(self, ws_strategy: str, ws_topk: int = 1):
        self.ws_strategy = ws_strategy
        self.ws_topk = ws_topk
...
def create_warm_starter(
    ...
    elif ws_strategy == 'random':
        return RandomConfigsWarmStart(ws_strategy, ws_topk)
    else:
        return NoWarmStart()
